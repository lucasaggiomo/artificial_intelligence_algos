...
class ProblemSolving:
    ...
    def simpleProblemSolvingAgent(self, search: SearchAlgorithmType, executeSolution : bool = False, print_problem: bool = False):
        ...
        print(f'Soluzione trovata (Costo {cost}):\n{'\n'.join(action.__str__() for action in actions)}')
    ...
    def uniformSearch(problem: Problem) -> SolutionType:
        ...
        fringeNodeMap: dict[State, Node] = {node.state: node}    # bruttissimo ma non ho altre idee
    ...
    @staticmethod
    def depthFirstSearchRecursiveHelper(problem: Problem, node: Node, explored: set) -> SolutionType:
        if problem.isGoalAchieved(node.state):
            return ProblemSolving.backtrackSolution(node)
    ...
    def depthFirstSearchRecursiveLimited(problem: Problem, limit: int) -> SolutionType:
        ...
        explored: set[State] = set()
        return ProblemSolving.depthFirstSearchRecursiveLimitedHelper(problem, initialNode, explored, limit)
    
    @staticmethod
    def depthFirstSearchRecursiveLimitedHelper(problem: Problem, node: Node, explored: set, limit: int) -> SolutionType:
        if problem.isGoalAchieved(node.state):
        ...
        for action in problem.getActionsFromState(node.state):
            ...
            if child.state not in explored:
                solution = ProblemSolving.depthFirstSearchRecursiveLimitedHelper(problem, child, explored, limit-1)
