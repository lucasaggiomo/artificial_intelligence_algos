...
class VacuumState(State):
    ...
    def __init__(self, bitmap: int):
        if bitmap < 0 or bitmap > 8:
            raise ValueError(f"{bitmap} is not a valid VacuumState value (valid values are from 0 to 8)")
        self.bitmap = bitmap
...
class VacuumAction(Action):
    ...
    def __init__(self, value: int):
        if value not in VacuumAction.ACTION_LABELS.keys():
            raise ValueError(f"{value} is not a valid VacuumAction value (valid values are: {VacuumAction.ACTION_LABELS})")
        self.value = value
...
def getNextBitmap(bitmap: int, action: VacuumAction) -> int:
    ...
    if action.value == VacuumAction.SUCK:
        return putBitInPosition(bitmap, VacuumState.CLEAN, (bitmap & VacuumState.VACUUM_MASK) >> VacuumState.VACUUM)
